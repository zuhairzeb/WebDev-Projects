...
export const SectionWrapper = ({ children, id, className, dark = false }: Props) => {
  return (
    <section 
      id={id} 
      className={`relative py-32 px-6 md:px-12 border-t border-gray-100 ${dark ? 'bg-black text-white' : 'bg-white'} ${className}`}
    >
      {/* Corner Markers */}
...
export const MarqueeText = ({ text, outline = false }: { text: string; outline?: boolean }) => {
  return (
    <div className="overflow-hidden whitespace-nowrap py-10 border-y border-gray-100 flex">
      <motion.div
        animate={{ x: [0, -1000] }}
        transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
        className={`text-8xl md:text-[12rem] font-bebas leading-none flex gap-8 pr-8 uppercase ${outline ? 'text-transparent stroke-black stroke-1' : ''}`}
